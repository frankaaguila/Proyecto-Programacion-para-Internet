<?php
	...
	function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT correo FROM usuario WHERE correo = ? LIMIT 1");
	...
	function registraUsuario($usuario, $pass_hash, $nombre, $email, $activo, $token, $tipo_usuario){
		...
		$stmt = $mysqli->prepare("INSERT INTO usuario (password, nombre, correo, activacion, token, id_tipo) VALUES(?,?,?,?,?,?,?)");
